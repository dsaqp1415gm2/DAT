...
function validar(){
	...
	$.ajax({
		...
		$.cookie('username', 'admin', { expires: 1 });
     		...
     		window.location = "Tecnologia_admin.html";
   		}
    		else{
		$("#modal_error").modal();
		}
...
$(document).ready(function(){
	var url = API_BASE_URL + '/dat/Theme/Tecnologia';
	getThreads(url);
});    
console.log('ok');
  }
...
function getThreads() {
    ...
    $.each(threads, function(i, v) {
        var thread = v;
        var url2 = API_BASE_URL+'/dat/Theme/Tecnologia/'+thread.idthread;
	var newPost;
